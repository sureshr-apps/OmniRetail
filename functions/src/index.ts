...
export const recordPasswordChange = onCall(callableOptions, async (request) => {
  try {
    const firebaseUid = requireVerifiedFirebaseIdentity(request.auth);
    await loadAuthorization(firebaseUid);
    console.info('Password change completed', { firebaseUid });
    return { recorded: true };
  } catch {
    throw new HttpsError('internal', 'Unable to record the password change operation.');
  }
});
...
export const provisionOrganizationAdministrator = onCall(callableOptions, async (request) => {
  try {
    ...
    const orchestrated = await orchestrateProvision({ organizationId, username, email, displayName, phone }, {
      async preflight() {
    ...
    try {
      ...
      console.info('Organization administrator onboarding email sent', { organizationId, appUserId: orchestrated.appUserId });
    ...
    if (error instanceof Error && /already in use|email already|already assigned|unique|duplicate|constraint/i.test(error.message)) throw new HttpsError('already-exists', 'A record with the same unique details already exists.');
...
export const changeOrganizationAdministratorStatus = onCall(callableOptions, async (request) => {
  try {
    ...
    try {
      await changeOrganizationAdministratorStatusSql({ organizationId, userId, status: status === 'active' ? AppUserStatus.ACTIVE : AppUserStatus.INACTIVE, membershipStatus: status === 'active' ? 'ACTIVE' as any : 'INACTIVE' as any });
    } catch {
      try { await auth.updateUser(targetUid, { disabled: previousStatus !== AppUserStatus.ACTIVE }); }
      catch { console.error('Failed to compensate Firebase Auth administrator status change', { organizationId, userId, errorClass: 'status_compensation_failed' }); }
...
export const assignOrganizationLicense = onCall(callableOptions, async (request) => {
  try {
    ...
    const licenseId = randomUUID(); const result = { licenseId, organizationId, planId, startDate, expiryDate, negotiatedPrice, currency };
    await assignOrganizationLicenseTrusted({ id: licenseId, organizationId, planId, startDate, expiryDate, negotiatedPrice, currency, historyId: randomUUID(), planCode: plan.planCode, planName: plan.name, planLevel: plan.level, maxStores: plan.maxStores, maxUsers: plan.maxUsers });
    return result;
  } catch (error) {
    logCallableFailure('assignOrganizationLicense', error);
    if (error instanceof Error && /already assigned|organization license|unique/i.test(error.message)) throw new HttpsError('already-exists', 'This organization already has a license.');
    throw new HttpsError('permission-denied', 'Unable to assign the organization license.');
  }
});
...
export const changeOrganizationLicensePlan = onCall(callableOptions, async (request) => {
  try {
    ...
    const result = { licenseId: current.id, organizationId, planId: target.id, startDate: current.startDate, expiryDate: current.expiryDate, negotiatedPrice, currency };
    await changeOrganizationLicensePlanTrusted({ id: current.id, organizationId, planId: target.id, startDate: current.startDate, expiryDate: current.expiryDate, negotiatedPrice, currency, historyId: randomUUID(), planCode: target.planCode, planName: target.name, planLevel: target.level, maxStores: target.maxStores, maxUsers: target.maxUsers, changes: { previousPlanId: currentPlan.id, previousPlanName: currentPlan.name, previousPlanLevel: currentPlan.level, previousMaxStores: currentPlan.maxStores, previousMaxUsers: currentPlan.maxUsers, previousNegotiatedPrice: current.negotiatedPrice, previousCurrency: current.currency } });
    return result;
...
export const deleteOrganizationLicensePlan = onCall(callableOptions, async (request) => {
  try {
    ...
    await deleteLicensePlanTrusted({ id });
    return { success: true };
  } catch (error) {
  ...
  try {
    ...
    for (const member of members) {
      ...
      try { await getAuth().deleteUser(member.user.firebaseUid); } catch (error: any) {
        if (error?.code !== 'auth/user-not-found') console.error('Failed to delete organization member Firebase Auth account', { organizationId, userId: member.user.id, errorClass: 'auth_delete_failed' });
      }
    }
    return { success: true };
  } catch (error) {
...
export const createTenantOutlet = onCall(callableOptions, async (request) => {
  try {
    ...
    const id = randomUUID();
    await createTenantOutletTrusted({ id, organizationId, name, contactPerson, email, phone, address });
...
export const updateTenantOutlet = onCall(callableOptions, async (request) => {
  try {
    ...
    await updateTenantOutletTrusted({ organizationId, id, name, contactPerson, email: typeof d.email === 'string' ? d.email.trim().toLowerCase() || null : null, phone, address });
    const row = (await getTenantOutletTrusted({ organizationId, id })).data.outlets[0];
...
export const changeTenantOutletStatus = onCall(callableOptions, async (request) => {
  try {
    ...
    await requireOrganizationAdmin(actorFirebaseUid, organizationId);
    await changeTenantOutletStatusTrusted({ organizationId, id, status });
    const row = (await getTenantOutletTrusted({ organizationId, id })).data.outlets[0];
...
export const deleteTenantOutlet = onCall(callableOptions, async (request) => {
  try {
    ...
    await requireOrganizationAdmin(actorFirebaseUid, organizationId);
    await deleteTenantOutletTrusted({ organizationId, id });
...
export const deleteTenantEmployee = onCall(callableOptions, async (request) => {
  try {
    ...
    const deleteInput = { organizationId, id };
    ...
    if (employee.user) {
      try {
        ...
        if (cleanupError?.code !== 'auth/user-not-found') {
          console.error('Failed to delete employee Firebase Auth account', { organizationId, employeeId: id, errorClass: 'auth_delete_failed' });
...
export const deleteTenantServicePerson = onCall(callableOptions, async (request) => {
  try {
    ...
    await deleteTenantServicePersonTrusted({ organizationId, id });
    return { success: true, organizationId, id };
  } catch (error) {
...
export const deleteTenantCustomer = onCall(callableOptions, async (request) => {
  try {
    ...
    if (!organizationId || !id || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input');
    await requireOrganizationAdmin(actorFirebaseUid, organizationId);
    await deleteTenantCustomerTrusted({ organizationId, id });
    return { success: true, organizationId, id };
  } catch (error) {
...
export const deleteTenantSupplier = onCall(callableOptions, async (request) => {
  try {
    ...
    if (!organizationId || !id || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input');
    await requireOrganizationAdmin(actorFirebaseUid, organizationId);
    await deleteTenantSupplierTrusted({ organizationId, id });
    return { success: true, organizationId, id };
  } catch (error) {
...
export const deleteTenantProduct = onCall(callableOptions, async (request) => {
  try {
    ...
    if (!organizationId || !id || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input');
    await requireOrganizationAdmin(actorFirebaseUid, organizationId);
    await deleteTenantProductTrusted({ organizationId, id });
    return { success: true, organizationId, id };
  } catch (error) {
...
export const deleteTenantCategory = onCall(callableOptions, async (request) => {
  try {
    ...
    if (!organizationId || !id || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input');
    await requireOrganizationAdmin(actorFirebaseUid, organizationId);
    await deleteTenantCategoryTrusted({ organizationId, id });
    return { success: true, organizationId, id };
  } catch (error) {
...
export const deleteTenantSubcategory = onCall(callableOptions, async (request) => {
  try {
    ...
    if (!organizationId || !id || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input');
    await requireOrganizationAdmin(actorFirebaseUid, organizationId);
    await deleteTenantSubcategoryTrusted({ organizationId, id });
    return { success: true, organizationId, id };
  } catch (error) {
...
function mapTrustedEmployeeRow(row: {
  ...
  designation: string; department?: string | null; gender?: string | null; dateOfBirth?: string | null; dateOfJoining: string;
  address?: string | null; notes?: string | null; assignmentScope: string;
  employmentStatus: string; loginAccess: string;
  ...
  return {
    ...
    phone: row.phone, designation: row.designation, department: row.department ?? null, gender: row.gender ?? null,
...
async function syncEmployeeOutletAssignment({
  organizationId,
  employeeId,
  assignmentScope,
  outletId,
  ...
  outletId: string;
  currentOutletIds: string[];
}): Promise<void> {
  for (const [index, currentOutletId] of currentOutletIds.entries()) {
    if (assignmentScope === 'OUTLET' && currentOutletId === outletId) continue;
    await deleteTenantEmployeeOutletTrusted({
      organizationId,
      employeeId,
      outletId: currentOutletId,
    });
  }
  if (assignmentScope === 'OUTLET') {
    await assignTenantEmployeeOutletTrusted({
      organizationId,
      employeeId,
      outletId,
    });
...
export const provisionTenantEmployee = onCall(callableOptions, async (request) => {
  ...
  try {
    ...
    const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const username = normalizeUsername(d.username); const email = employeeAuthEmail(username); const initialPassword = typeof d.initialPassword === 'string' ? d.initialPassword : ''; const permissionProfile = d.permissionProfile === 'Admin' ? 'Admin' : d.permissionProfile === 'User' || d.permissionProfile === undefined ? 'User' : ''; const fullName = typeof d.fullName === 'string' ? d.fullName.trim() : ''; const phone = typeof d.phone === 'string' ? d.phone.trim() : ''; const designation = typeof d.designation === 'string' ? d.designation.trim() : ''; const gender = typeof d.gender === 'string' ? d.gender.trim() || null : null; const assignmentScope = d.assignmentScope === 'OUTLET' ? 'OUTLET' : 'ORGANIZATION'; const outletId = typeof d.outletId === 'string' ? d.outletId : ''; const dateOfBirth = typeof d.dateOfBirth === 'string' && d.dateOfBirth ? d.dateOfBirth : null; const dateOfJoining = typeof d.dateOfJoining === 'string' ? d.dateOfJoining : ''; const address = typeof d.address === 'string' ? d.address.trim() || null : null; const notes = typeof d.notes === 'string' ? d.notes.trim() || null : null;
    const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : '';
    if (!organizationId || !username || !permissionProfile || initialPassword.length < 6 || initialPassword.length > 4096 || !fullName || !phone || !designation || (assignmentScope === 'OUTLET' && !outletId) || (dateOfBirth !== null && !/^\d{4}-\d{2}-\d{2}$/.test(dateOfBirth)) || !/^\d{4}-\d{2}-\d{2}$/.test(dateOfJoining) || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input');
    await requireOrganizationAdmin(actorFirebaseUid, organizationId);
    ...
    await provisionTenantEmployeeTrusted({ id: employeeId, userId: randomUUID(), firebaseUid: created.uid, username, email, organizationId, fullName, phone, designation, department: typeof d.department === 'string' ? d.department.trim() || null : null, gender, dateOfBirth, dateOfJoining, address, notes, assignmentScope, roleId: permissionProfile === 'Admin' ? '00000000-0000-4000-8000-000000000002' : '00000000-0000-4000-8000-000000000003' });
    if (assignmentScope === 'OUTLET') await syncEmployeeOutletAssignment({ organizationId, employeeId, assignmentScope, outletId, currentOutletIds: [] });
...
export const updateTenantEmployeeLogin = onCall(callableOptions, async (request) => {
  ...
  try {
    ...
    if (!email || !currentUser?.firebaseUid) {
      ...
      await provisionTenantEmployeeLoginTrusted({ organizationId, employeeId, userId: randomUUID(), firebaseUid: created.uid, username, email, displayName: current.fullName, phone: current.phone || null, roleId: employeeRoleId(permissionProfile) });
    } else {
      ...
      await updateTenantEmployeeLoginTrusted({ organizationId, employeeId, userId: currentUser.id, username: username || currentUsername, email: email || currentEmail, roleId: employeeRoleId(permissionProfile), loginAccess: allowLogin ? LoginAccessStatus.ENABLED : LoginAccessStatus.DISABLED });
    }
    const row = (await getTenantEmployeeTrusted({ organizationId, id: employeeId })).data.employees[0];
...
export const createTenantEmployeeProfile = onCall(callableOptions, async (request) => {
  try {
    const actor = requireVerifiedFirebaseIdentity(request.auth); const caller = await loadAuthorization(actor); requireCapability(caller, 'employees.read'); const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const fullName = typeof d.fullName === 'string' ? d.fullName.trim() : ''; const phone = typeof d.phone === 'string' ? d.phone.trim() : ''; const designation = typeof d.designation === 'string' ? d.designation.trim() : ''; const gender = typeof d.gender === 'string' ? d.gender.trim() || null : null; const assignmentScope = d.assignmentScope === 'OUTLET' ? 'OUTLET' : 'ORGANIZATION'; const outletId = typeof d.outletId === 'string' ? d.outletId : ''; const dateOfBirth = typeof d.dateOfBirth === 'string' && d.dateOfBirth ? d.dateOfBirth : null; const dateOfJoining = typeof d.dateOfJoining === 'string' ? d.dateOfJoining : ''; const address = typeof d.address === 'string' ? d.address.trim() || null : null; const notes = typeof d.notes === 'string' ? d.notes.trim() || null : null; const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : '';
    if (!organizationId || !fullName || !phone || !designation || (assignmentScope === 'OUTLET' && !outletId) || (dateOfBirth !== null && !/^\d{4}-\d{2}-\d{2}$/.test(dateOfBirth)) || !/^\d{4}-\d{2}-\d{2}$/.test(dateOfJoining) || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationAdmin(actor, organizationId);
    const id = randomUUID();
    await createTenantEmployeeProfileTrusted({ id, organizationId, fullName, email: typeof d.email === 'string' ? d.email.trim().toLowerCase() || null : null, phone, designation, department: typeof d.department === 'string' ? d.department.trim() || null : null, gender, dateOfBirth, dateOfJoining, address, notes, assignmentScope });
    if (assignmentScope === 'OUTLET') await syncEmployeeOutletAssignment({ organizationId, employeeId: id, assignmentScope, outletId, currentOutletIds: [] });
...
export const updateTenantEmployee = onCall(callableOptions, async (request) => {
  try {
    ...
    const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const id = typeof d.id === 'string' ? d.id : ''; const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : ''; const fullName = typeof d.fullName === 'string' ? d.fullName.trim() : ''; const phone = typeof d.phone === 'string' ? d.phone.trim() : ''; const designation = typeof d.designation === 'string' ? d.designation.trim() : ''; const gender = typeof d.gender === 'string' ? d.gender.trim() || null : null; const assignmentScope = d.assignmentScope === 'OUTLET' ? 'OUTLET' : 'ORGANIZATION'; const outletId = typeof d.outletId === 'string' ? d.outletId : ''; const dateOfBirth = typeof d.dateOfBirth === 'string' && d.dateOfBirth ? d.dateOfBirth : null; const dateOfJoining = typeof d.dateOfJoining === 'string' ? d.dateOfJoining : ''; const address = typeof d.address === 'string' ? d.address.trim() || null : null; const notes = typeof d.notes === 'string' ? d.notes.trim() || null : null;
    if (!organizationId || !id || !fullName || !phone || !designation || (assignmentScope === 'OUTLET' && !outletId) || (dateOfBirth !== null && !/^\d{4}-\d{2}-\d{2}$/.test(dateOfBirth)) || !/^\d{4}-\d{2}-\d{2}$/.test(dateOfJoining) || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input');
    await requireOrganizationAdmin(actor, organizationId);
    const current = (await getTenantEmployeeTrusted({ organizationId, id })).data.employees[0];
    if (!current) throw new Error('employee not found');
    const currentOutletIds = current.employeeOutlets_on_employee.map((assignment) => assignment.outlet.id);
    await updateTenantEmployeeTrusted({ organizationId, id, fullName, email: typeof d.email === 'string' ? d.email.trim().toLowerCase() || null : null, phone, designation, department: typeof d.department === 'string' ? d.department.trim() || null : null, gender, dateOfBirth, dateOfJoining, address, notes, assignmentScope });
    await syncEmployeeOutletAssignment({ organizationId, employeeId: id, assignmentScope, outletId, currentOutletIds });
    const row = (await getTenantEmployeeTrusted({ organizationId, id })).data.employees[0];
...
export const changeTenantEmployeeLoginAccess = onCall(callableOptions, async (request) => {
  ...
  try {
    ...
    await changeTenantEmployeeLoginAccessTrusted({ organizationId, id, userId: employee.user.id, loginAccess: enabled ? LoginAccessStatus.ENABLED : LoginAccessStatus.DISABLED });
    const row = (await getTenantEmployeeTrusted({ organizationId, id })).data.employees[0];
...
export const createTenantServicePerson = onCall(callableOptions, async (request) => {
  try {
    const actor = requireVerifiedFirebaseIdentity(request.auth); const caller = await loadAuthorization(actor); requireCapability(caller, 'service_persons.read'); const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const fullName = typeof d.fullName === 'string' ? d.fullName.trim() : ''; const phone = typeof d.phone === 'string' ? d.phone.trim() : ''; const address = typeof d.address === 'string' ? d.address.trim() || null : null; const specialization = typeof d.specialization === 'string' ? d.specialization.trim() || null : null; const notes = typeof d.notes === 'string' ? d.notes.trim() || null : null; const assignmentScope = d.assignmentScope === 'OUTLET' ? 'OUTLET' : 'ORGANIZATION'; const outletId = typeof d.outletId === 'string' ? d.outletId : ''; const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : '';
    if (!organizationId || !fullName || !phone || (assignmentScope === 'OUTLET' && !outletId) || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationAdmin(actor, organizationId);
    const id = randomUUID();
    await createTenantServicePersonTrusted({ id, organizationId, fullName, email: typeof d.email === 'string' ? d.email.trim().toLowerCase() || null : null, phone, address, specialization, yearsOfExperience: Number.isInteger(d.yearsOfExperience) ? d.yearsOfExperience : null, assignmentScope, notes });
    if (assignmentScope === 'OUTLET') await assignTenantServicePersonOutletTrusted({ organizationId, servicePersonId: id, outletId });
...
export const changeTenantProductStatus = onCall(callableOptions, async (request) => {
  try { const actor = requireVerifiedFirebaseIdentity(request.auth); const caller = await loadAuthorization(actor); requireCapability(caller, 'products.read'); const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const id = typeof d.id === 'string' ? d.id : ''; const status = d.status === 'ACTIVE' || d.status === 'INACTIVE' ? d.status : ''; const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : ''; if (!organizationId || !id || !status || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationCapability(actor, organizationId, 'products.read'); await changeTenantProductStatusSql({ organizationId, id, status }); const row = (await getTenantProductTrusted({ organizationId, id })).data.products[0]; if (!row) throw new Error('product not found after status change'); return { success: true, organizationId, ...row }; } catch (error) { logCallableFailure('changeTenantProductStatus', error); throw new HttpsError('permission-denied', 'Unable to change the product status.'); }
});

export const createTenantCustomerRecord = onCall(callableOptions, async (request) => {
    try { const actor = requireVerifiedFirebaseIdentity(request.auth); const caller = await loadAuthorization(actor); requireCapability(caller, 'customers.read'); const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const name = typeof d.name === 'string' ? d.name.trim() : ''; const phone = typeof d.phone === 'string' ? d.phone.trim() : ''; const email = typeof d.email === 'string' ? d.email.trim().toLowerCase() || null : null; const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : ''; const type: CustomerType = d.type === 'BUSINESS' ? CustomerType.BUSINESS : CustomerType.INDIVIDUAL; if (!organizationId || !name || !phone || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationCapability(actor, organizationId, 'customers.read'); const id = randomUUID(); await createTenantCustomer({ id, organizationId, type, name, phone, email, taxId: typeof d.taxId === 'string' ? d.taxId.trim() || null : null, documentType: typeof d.documentType === 'string' ? d.documentType.trim() || null : null, documentValue: typeof d.documentValue === 'string' ? d.documentValue.trim() || null : null, address: typeof d.address === 'string' ? d.address.trim() || null : null, creditLimit: Number.isFinite(Number(d.creditLimit)) ? Number(d.creditLimit) : null, preferredContact: null, dateOfBirth: typeof d.dateOfBirth === 'string' ? d.dateOfBirth : null, gender: typeof d.gender === 'string' ? d.gender.trim() || null : null, notes: typeof d.notes === 'string' ? d.notes.trim() || null : null }); const row = (await getTenantCustomerTrusted({ organizationId, id })).data.customers[0]; if (!row) throw new Error('customer not found after creation'); return { success: true, organizationId, ...row }; } catch (error) { logCallableFailure('createTenantCustomerRecord', error); throw new HttpsError('permission-denied', 'Unable to create the customer.'); }
});

export const updateTenantCustomerRecord = onCall(callableOptions, async (request) => {
  try { const actor = requireVerifiedFirebaseIdentity(request.auth); const caller = await loadAuthorization(actor); requireCapability(caller, 'customers.read'); const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const id = typeof d.id === 'string' ? d.id : ''; const name = typeof d.name === 'string' ? d.name.trim() : ''; const phone = typeof d.phone === 'string' ? d.phone.trim() : ''; const email = typeof d.email === 'string' ? d.email.trim().toLowerCase() || null : null; const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : ''; const type: CustomerType = d.type === 'BUSINESS' ? CustomerType.BUSINESS : CustomerType.INDIVIDUAL; if (!organizationId || !id || !name || !phone || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationCapability(actor, organizationId, 'customers.read'); await updateTenantCustomer({ organizationId, id, type, name, phone, email, taxId: typeof d.taxId === 'string' ? d.taxId.trim() || null : null, documentType: typeof d.documentType === 'string' ? d.documentType.trim() || null : null, documentValue: typeof d.documentValue === 'string' ? d.documentValue.trim() || null : null, address: typeof d.address === 'string' ? d.address.trim() || null : null, creditLimit: Number.isFinite(Number(d.creditLimit)) ? Number(d.creditLimit) : null, preferredContact: null, dateOfBirth: typeof d.dateOfBirth === 'string' ? d.dateOfBirth : null, gender: typeof d.gender === 'string' ? d.gender.trim() || null : null, notes: typeof d.notes === 'string' ? d.notes.trim() || null : null }); const row = (await getTenantCustomerTrusted({ organizationId, id })).data.customers[0]; if (!row) throw new Error('customer not found after update'); return { success: true, organizationId, ...row }; } catch (error) { logCallableFailure('updateTenantCustomerRecord', error); throw new HttpsError('permission-denied', 'Unable to update the customer.'); }
});

export const changeTenantCustomerStatus = onCall(callableOptions, async (request) => {
  try { const actor = requireVerifiedFirebaseIdentity(request.auth); const caller = await loadAuthorization(actor); requireCapability(caller, 'customers.read'); const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const id = typeof d.id === 'string' ? d.id : ''; const status = d.status === 'ACTIVE' || d.status === 'INACTIVE' ? d.status : ''; const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : ''; if (!organizationId || !id || !status || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationCapability(actor, organizationId, 'customers.read'); await changeTenantCustomerStatusSql({ organizationId, id, status }); const row = (await getTenantCustomerTrusted({ organizationId, id })).data.customers[0]; if (!row) throw new Error('customer not found after status change'); return { success: true, organizationId, ...row }; } catch (error) { logCallableFailure('changeTenantCustomerStatus', error); throw new HttpsError('permission-denied', 'Unable to change the customer status.'); }
});

export const createTenantSupplierRecord = onCall(callableOptions, async (request) => {
  try { const actor = requireVerifiedFirebaseIdentity(request.auth); const caller = await loadAuthorization(actor); requireCapability(caller, 'suppliers.read'); const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const name = typeof d.name === 'string' ? d.name.trim() : ''; const contactPerson = typeof d.contactPerson === 'string' ? d.contactPerson.trim() : ''; const phone = typeof d.phone === 'string' ? d.phone.trim() : ''; const email = typeof d.email === 'string' ? d.email.trim().toLowerCase() : ''; const taxId = typeof d.taxId === 'string' ? d.taxId.trim() || null : null; const category = typeof d.category === 'string' ? d.category.trim() : ''; const paymentTerms = typeof d.paymentTerms === 'string' ? d.paymentTerms.trim() : ''; const creditLimit = Number(d.creditLimit); const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : ''; if (!organizationId || !name || !contactPerson || !phone || !email || !category || !paymentTerms || !Number.isFinite(creditLimit) || creditLimit < 0 || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationCapability(actor, organizationId, 'suppliers.read'); const id = randomUUID(); await createTenantSupplier({ id, organizationId, name, contactPerson, phone, email, taxId, address: typeof d.address === 'string' ? d.address.trim() || null : null, category, paymentTerms, creditLimit, notes: typeof d.notes === 'string' ? d.notes.trim() || null : null }); const row = (await getTenantSupplierTrusted({ organizationId, id })).data.suppliers[0]; if (!row) throw new Error('supplier not found after creation'); return { success: true, organizationId, ...row }; } catch (error) { logCallableFailure('createTenantSupplierRecord', error); throw new HttpsError('permission-denied', 'Unable to create the supplier.'); }
});

export const changeTenantSupplierStatus = onCall(callableOptions, async (request) => {
  try { const actor = requireVerifiedFirebaseIdentity(request.auth); const caller = await loadAuthorization(actor); requireCapability(caller, 'suppliers.read'); const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const id = typeof d.id === 'string' ? d.id : ''; const status = d.status === 'ACTIVE' || d.status === 'INACTIVE' ? d.status : ''; const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : ''; if (!organizationId || !id || !status || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationCapability(actor, organizationId, 'suppliers.read'); await changeTenantSupplierStatusSql({ organizationId, id, status }); const row = (await getTenantSupplierTrusted({ organizationId, id })).data.suppliers[0]; if (!row) throw new Error('supplier not found after status change'); return { success: true, organizationId, ...row }; } catch (error) { logCallableFailure('changeTenantSupplierStatus', error); throw new HttpsError('permission-denied', 'Unable to change supplier status.'); }
});

export const updateTenantSupplierRecord = onCall(callableOptions, async (request) => {
  try { const actor = requireVerifiedFirebaseIdentity(request.auth); const caller = await loadAuthorization(actor); requireCapability(caller, 'suppliers.read'); const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const id = typeof d.id === 'string' ? d.id : ''; const required = (key: string) => typeof d[key] === 'string' ? d[key].trim() : ''; const taxId = typeof d.taxId === 'string' ? d.taxId.trim() || null : null; const creditLimit = Number(d.creditLimit); const requestId = required('requestId'); if (!organizationId || !id || !required('name') || !required('contactPerson') || !required('phone') || !required('email') || !required('category') || !required('paymentTerms') || !Number.isFinite(creditLimit) || creditLimit < 0 || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationCapability(actor, organizationId, 'suppliers.read'); await updateTenantSupplier({ organizationId, id, name: required('name'), contactPerson: required('contactPerson'), phone: required('phone'), email: required('email').toLowerCase(), taxId, address: required('address') || null, category: required('category'), paymentTerms: required('paymentTerms'), creditLimit, notes: required('notes') || null }); const row = (await getTenantSupplierTrusted({ organizationId, id })).data.suppliers[0]; if (!row) throw new Error('supplier not found after update'); return { success: true, organizationId, ...row }; } catch (error) { logCallableFailure('updateTenantSupplierRecord', error); throw new HttpsError('permission-denied', 'Unable to update the supplier.'); }
});

export const createTenantPurchaseRecord = onCall(callableOptions, async (request) => {
  try { const actor = requireVerifiedFirebaseIdentity(request.auth); const caller = await loadAuthorization(actor); requireCapability(caller, 'purchases.read'); const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const purchaseNumber = typeof d.purchaseNumber === 'string' ? d.purchaseNumber.trim() : ''; const supplierId = typeof d.supplierId === 'string' ? d.supplierId : ''; const purchaseDate = typeof d.purchaseDate === 'string' ? d.purchaseDate : ''; const createdBy = typeof d.createdBy === 'string' ? d.createdBy.trim() : ''; const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : ''; const n = (key: string) => Number(d[key]); const lines = Array.isArray(d.lines) ? d.lines : []; const outletId = typeof d.outletId === 'string' ? d.outletId : ''; const status = d.status === 'DRAFT' || d.status === 'ACTIVE' ? d.status : ''; const paymentStatus = d.paymentStatus === 'PAID' || d.paymentStatus === 'PARTIALLY_PAID' || d.paymentStatus === 'UNPAID' ? d.paymentStatus : ''; const totalAmount = n('totalAmount'); const amountPaid = n('amountPaid'); const suppliedOutstandingAmount = n('outstandingAmount'); const outstandingAmount = Math.max(0, Number((totalAmount - amountPaid).toFixed(2))); if (!organizationId || !purchaseNumber || !supplierId || !/^\d{4}-\d{2}-\d{2}$/.test(purchaseDate) || !createdBy || !status || !paymentStatus || !Number.isFinite(n('subtotal')) || !Number.isFinite(n('shippingFee')) || !Number.isFinite(n('handlingFee')) || !Number.isFinite(n('tax')) || !Number.isFinite(totalAmount) || !Number.isFinite(amountPaid) || !Number.isFinite(suppliedOutstandingAmount) || lines.length === 0 || !lines.every((line: any) => line && typeof line.productId === 'string' && Number.isFinite(Number(line.quantity)) && Number(line.quantity) > 0) || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationCapability(actor, organizationId, 'purchases.read'); if (outletId) { const outlet = (await getTenantOutletTrusted({ organizationId, id: outletId })).data.outlets[0]; if (!outlet || outlet.status !== 'ACTIVE') throw new Error('outlet is not in this organization'); } const createdPurchase = await createTenantPurchase({ organizationId, purchaseNumber, purchaseDate, supplierId, outletId: outletId || null, scope: typeof d.scope === 'string' ? d.scope : 'outlet', paymentTerms: typeof d.paymentTerms === 'string' ? d.paymentTerms : null, subtotal: n('subtotal'), shippingFee: n('shippingFee'), handlingFee: n('handlingFee'), tax: n('tax'), totalAmount, amountPaid, outstandingAmount, paymentStatus, receiptStatus: PurchaseReceiptStatus.PENDING, status, createdBy }); for (const line of lines as any[]) { const quantity = Number(line.quantity); const unitCost = Number(line.unitCost); const discountPercent = Number(line.discountPercent ?? 0); const taxRate = Number(line.taxRate ?? 0); const taxAmount = Number(line.taxAmount ?? 0); const lineTotal = Number(line.lineTotal ?? quantity * unitCost); if (![unitCost, discountPercent, taxRate, taxAmount, lineTotal].every(Number.isFinite) || unitCost < 0 || lineTotal < 0) throw new Error('invalid line'); await createTenantPurchaseLine({ organizationId, purchaseId: createdPurchase.data.purchase_insert.id, productId: line.productId, quantityOrdered: quantity, unitCost, discountPercent, taxRate, taxAmount, lineTotal }); } return { success: true, organizationId, purchaseNumber }; } catch (error) { logCallableFailure('createTenantPurchaseRecord', error); throw new HttpsError('permission-denied', 'Unable to create the purchase.'); }
});
...
export const receiveTenantPurchaseLineRecord = onCall(callableOptions, async (request) => {
  try {
    ...
    const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const purchaseId = typeof d.purchaseId === 'string' ? d.purchaseId : ''; const lineId = typeof d.lineId === 'string' ? d.lineId : ''; const quantityReceived = Number(d.quantityReceived); const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : '';
    if (!organizationId || !purchaseId || !lineId || !Number.isFinite(quantityReceived) || quantityReceived <= 0 || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input');
    await requireOrganizationCapability(actor, organizationId, 'purchases.read');
...
export const resendAdministratorOnboardingEmail = onCall(callableOptions, async (request) => {
  try { const t = await authorizeTarget(request, 'organization_admins.create'); await sendManagedPasswordEmail(t.identity.user.email, 'PASSWORD_RESET'); console.info('Organization administrator onboarding email resent', { organizationId: t.organizationId, appUserId: t.appUserId }); return { success: true }; }
...
export const resetOrganizationAdministratorPassword = onCall(callableOptions, async (request) => {
  try {
    ...
    console.info('Organization administrator password reset initiated', { organizationId: t.organizationId, appUserId: t.appUserId });
    return { success: true };
