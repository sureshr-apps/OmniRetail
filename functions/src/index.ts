...
export const assignOrganizationLicense = onCall(callableOptions, async (request) => {
  try {
    ...
    if ((await getOrganizationLicense({ organizationId })).data.organizationLicenses.length) throw new Error('already assigned');
...
export const changeOrganizationLicensePlan = onCall(callableOptions, async (request) => {
  try {
    ...
    const current = (await getOrganizationLicense({ organizationId })).data.organizationLicenses[0]; if (!current) throw new Error('license');
...
export const renewOrganizationLicense = onCall(callableOptions, async (request) => {
  try {
    ...
    const current = (await getOrganizationLicense({ organizationId })).data.organizationLicenses[0]; if (!current || new Date(newStartDate) <= new Date(current.expiryDate) || new Date(newExpiryDate) <= new Date(newStartDate)) throw new Error('invalid term');
