...
export const createTenantServicePerson = onCall(callableOptions, async (request) => {
  try {
    ...
    const id = randomUUID();
    await createTenantServicePersonTrusted({ id, organizationId, fullName, email: typeof d.email === 'string' ? d.email.trim().toLowerCase() || null : null, phone, specialization, yearsOfExperience: Number.isInteger(d.yearsOfExperience) ? d.yearsOfExperience : null, assignmentScope: typeof d.assignmentScope === 'string' ? d.assignmentScope : 'ORGANIZATION', auditId: randomUUID(), requestId, actorFirebaseUid: actor });
