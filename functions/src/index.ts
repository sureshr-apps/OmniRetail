...
export const createTenantServicePerson = onCall(callableOptions, async (request) => {
  try {
    ...
    const id = randomUUID();
    await createTenantServicePersonTrusted({ id, organizationId, fullName, email: typeof d.email === 'string' ? d.email.trim().toLowerCase() || null : null, phone, specialization, yearsOfExperience: Number.isInteger(d.yearsOfExperience) ? d.yearsOfExperience : null, assignmentScope, notes, auditId: randomUUID(), requestId, actorFirebaseUid: actor });
    if (assignmentScope === 'OUTLET') await assignTenantServicePersonOutletTrusted({ organizationId, servicePersonId: id, outletId, auditId: randomUUID(), requestId, actorFirebaseUid: actor });
