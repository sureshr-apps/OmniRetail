...
export const createTenantServicePerson = onCall(callableOptions, async (request) => {
  try {
    ...
    if (!organizationId || !fullName || !phone || !specialization || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationAdmin(actor, organizationId);
    const id = randomUUID();
