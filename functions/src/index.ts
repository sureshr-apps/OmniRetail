...
import { derivePurchasePaymentStatus, recordPurchasePayment } from './purchasePayments.js';
...
export const createTenantPurchaseRecord = onCall(callableOptions, async (request) => {
  try {
    ...
    const d = request.data ?? {};
    const organizationId = typeof d.organizationId === 'string' ? d.organizationId : '';
    const purchaseNumber = typeof d.purchaseNumber === 'string' ? d.purchaseNumber.trim() : '';
    const supplierId = typeof d.supplierId === 'string' ? d.supplierId : '';
    const purchaseDate = typeof d.purchaseDate === 'string' ? d.purchaseDate : '';
    const createdBy = typeof d.createdBy === 'string' ? d.createdBy.trim() : '';
    const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : '';
    const n = (key: string) => Number(d[key]);
    const lines = Array.isArray(d.lines) ? d.lines : [];
    const outletId = typeof d.outletId === 'string' ? d.outletId : '';
    const status = d.status === 'DRAFT' || d.status === 'ACTIVE' ? d.status : '';
    const totalAmount = n('totalAmount');
    const amountPaid = n('amountPaid');
    const suppliedOutstandingAmount = n('outstandingAmount');
    const outstandingAmount = Math.max(0, Number((totalAmount - amountPaid).toFixed(2)));
