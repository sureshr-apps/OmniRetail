...
export const createTenantCustomerRecord = onCall(callableOptions, async (request) => {
    try { const actor = requireVerifiedFirebaseIdentity(request.auth); const caller = await loadAuthorization(actor); requireCapability(caller, 'customers.read'); const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const name = typeof d.name === 'string' ? d.name.trim() : ''; const phone = typeof d.phone === 'string' ? d.phone.trim() : ''; const email = typeof d.email === 'string' ? d.email.trim().toLowerCase() : ''; const city = typeof d.city === 'string' ? d.city.trim() : ''; const state = typeof d.state === 'string' ? d.state.trim() : ''; const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : ''; const type: CustomerType = d.type === 'BUSINESS' ? CustomerType.BUSINESS : CustomerType.INDIVIDUAL; if (!organizationId || !name || !phone || !email || !city || !state || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationCapability(actor, organizationId, 'customers.read'); const id = randomUUID(); await createTenantCustomer({ id, organizationId, type, name, phone, email, taxId: typeof d.taxId === 'string' ? d.taxId.trim() || null : null, documentType: typeof d.documentType === 'string' ? d.documentType.trim() || null : null, documentValue: typeof d.documentValue === 'string' ? d.documentValue.trim() || null : null, address: typeof d.address === 'string' ? d.address.trim() || null : null, city, state, postalCode: typeof d.postalCode === 'string' ? d.postalCode.trim() || null : null, country: typeof d.country === 'string' ? d.country.trim() || null : null, creditLimit: Number.isFinite(Number(d.creditLimit)) ? Number(d.creditLimit) : null, preferredContact: null, dateOfBirth: typeof d.dateOfBirth === 'string' ? d.dateOfBirth : null, gender: typeof d.gender === 'string' ? d.gender.trim() || null : null, notes: typeof d.notes === 'string' ? d.notes.trim() || null : null }); const row = (await getTenantCustomerTrusted({ organizationId, id })).data.customers[0]; if (!row) throw new Error('customer not found after creation'); return { success: true, organizationId, ...row }; } catch (error) { logCallableFailure('createTenantCustomerRecord', error); throw new HttpsError('permission-denied', 'Unable to create the customer.'); }
});

export const updateTenantCustomerRecord = onCall(callableOptions, async (request) => {
  try { const actor = requireVerifiedFirebaseIdentity(request.auth); const caller = await loadAuthorization(actor); requireCapability(caller, 'customers.read'); const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const id = typeof d.id === 'string' ? d.id : ''; const name = typeof d.name === 'string' ? d.name.trim() : ''; const phone = typeof d.phone === 'string' ? d.phone.trim() : ''; const email = typeof d.email === 'string' ? d.email.trim().toLowerCase() : ''; const city = typeof d.city === 'string' ? d.city.trim() : ''; const state = typeof d.state === 'string' ? d.state.trim() : ''; const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : ''; const type: CustomerType = d.type === 'BUSINESS' ? CustomerType.BUSINESS : CustomerType.INDIVIDUAL; if (!organizationId || !id || !name || !phone || !email || !city || !state || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationCapability(actor, organizationId, 'customers.read'); await updateTenantCustomer({ organizationId, id, type, name, phone, email, taxId: typeof d.taxId === 'string' ? d.taxId.trim() || null : null, documentType: typeof d.documentType === 'string' ? d.documentType.trim() || null : null, documentValue: typeof d.documentValue === 'string' ? d.documentValue.trim() || null : null, address: typeof d.address === 'string' ? d.address.trim() || null : null, city, state, postalCode: typeof d.postalCode === 'string' ? d.postalCode.trim() || null : null, country: typeof d.country === 'string' ? d.country.trim() || null : null, creditLimit: Number.isFinite(Number(d.creditLimit)) ? Number(d.creditLimit) : null, preferredContact: null, dateOfBirth: typeof d.dateOfBirth === 'string' ? d.dateOfBirth : null, gender: typeof d.gender === 'string' ? d.gender.trim() || null : null, notes: typeof d.notes === 'string' ? d.notes.trim() || null : null }); const row = (await getTenantCustomerTrusted({ organizationId, id })).data.customers[0]; if (!row) throw new Error('customer not found after update'); return { success: true, organizationId, ...row }; } catch (error) { logCallableFailure('updateTenantCustomerRecord', error); throw new HttpsError('permission-denied', 'Unable to update the customer.'); }
});
...
export const createTenantSupplierRecord = onCall(callableOptions, async (request) => {
  try { const actor = requireVerifiedFirebaseIdentity(request.auth); const caller = await loadAuthorization(actor); requireCapability(caller, 'suppliers.read'); const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const name = typeof d.name === 'string' ? d.name.trim() : ''; const contactPerson = typeof d.contactPerson === 'string' ? d.contactPerson.trim() : ''; const phone = typeof d.phone === 'string' ? d.phone.trim() : ''; const email = typeof d.email === 'string' ? d.email.trim().toLowerCase() : ''; const taxId = typeof d.taxId === 'string' ? d.taxId.trim() : ''; const city = typeof d.city === 'string' ? d.city.trim() : ''; const category = typeof d.category === 'string' ? d.category.trim() : ''; const paymentTerms = typeof d.paymentTerms === 'string' ? d.paymentTerms.trim() : ''; const creditLimit = Number(d.creditLimit); const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : ''; if (!organizationId || !name || !contactPerson || !phone || !email || !taxId || !city || !category || !paymentTerms || !Number.isFinite(creditLimit) || creditLimit < 0 || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationCapability(actor, organizationId, 'suppliers.read'); const id = randomUUID(); await createTenantSupplier({ id, organizationId, name, contactPerson, phone, email, taxId, address: typeof d.address === 'string' ? d.address.trim() || null : null, city, state: typeof d.state === 'string' ? d.state.trim() || null : null, postalCode: typeof d.postalCode === 'string' ? d.postalCode.trim() || null : null, country: typeof d.country === 'string' ? d.country.trim() || null : null, category, paymentTerms, creditLimit, notes: typeof d.notes === 'string' ? d.notes.trim() || null : null }); const row = (await getTenantSupplierTrusted({ organizationId, id })).data.suppliers[0]; if (!row) throw new Error('supplier not found after creation'); return { success: true, organizationId, ...row }; } catch (error) { logCallableFailure('createTenantSupplierRecord', error); throw new HttpsError('permission-denied', 'Unable to create the supplier.'); }
});
