...
export const createTenantServicePerson = onCall(callableOptions, async (request) => {
  try {
    const actor = requireVerifiedFirebaseIdentity(request.auth); const caller = await loadAuthorization(actor); requireCapability(caller, 'service_persons.read'); const d = request.data ?? {}; const organizationId = typeof d.organizationId === 'string' ? d.organizationId : ''; const fullName = typeof d.fullName === 'string' ? d.fullName.trim() : ''; const phone = typeof d.phone === 'string' ? d.phone.trim() : ''; const specialization = typeof d.specialization === 'string' ? d.specialization.trim() || null : null; const notes = typeof d.notes === 'string' ? d.notes.trim() || null : null; const requestId = typeof d.requestId === 'string' ? d.requestId.trim() : '';
    if (!organizationId || !fullName || !phone || !/^[A-Za-z0-9._:-]{8,128}$/.test(requestId)) throw new Error('invalid input'); await requireOrganizationAdmin(actor, organizationId);
    const id = randomUUID();
    await createTenantServicePersonTrusted({ id, organizationId, fullName, email: typeof d.email === 'string' ? d.email.trim().toLowerCase() || null : null, phone, specialization, yearsOfExperience: Number.isInteger(d.yearsOfExperience) ? d.yearsOfExperience : null, assignmentScope: typeof d.assignmentScope === 'string' ? d.assignmentScope : 'ORGANIZATION', notes, auditId: randomUUID(), requestId, actorFirebaseUid: actor });
