...
class ProductionEmployeeService implements IEmployeeService {
  ...
  public getNextEmployeeCode(): string { return `EMP-${Date.now().toString().slice(-8)}`; }
  ...
  public async getEmployee(id: string): Promise<Employee | null> { const result = await this.getEmployees({ page: 1, pageSize: 1000 }); return result.employees.find((row) => row.id === id || row.employeeCode === id) ?? null; }
  public async getDepartments(): Promise<string[]> { return Array.from(new Set((await this.allProduction()).map((employee) => employee.department).filter(Boolean))).sort(); }
  public async getDesignations(): Promise<string[]> { return Array.from(new Set((await this.allProduction()).map((employee) => employee.designation).filter(Boolean))).sort(); }
