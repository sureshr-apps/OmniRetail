...
export function deriveEmployeeView(all: Employee[], query: EmployeeQuery): EmployeeQueryResult {
  ...
  if (search) rows = rows.filter((row) => `${row.employeeCode} ${row.displayName} ${row.email} ${row.phone} ${row.designation}`.toLowerCase().includes(search));
...
class ProductionEmployeeService implements IEmployeeService {
  ...
  public async getEmployee(id: string): Promise<Employee | null> { const all = await this.getAllEmployees(); return all.find((row) => row.id === id || row.employeeCode === id) ?? null; }
  ...
  public async createEmployee(input: CreateEmployeeInput): Promise<Employee> {
    ...
    const employeeCode = this.getNextEmployeeCode();
    const payload = {
      organizationId,
      employeeCode,
      fullName,
