...
export function deriveOrganizationView(all: Organization[], q: OrganizationQuery): PaginatedResult<Organization> {
  ...
  if (search) items = items.filter((organization) => `${organization.id} ${organization.name} ${organization.contactInfo.primaryContactName} ${organization.contactInfo.city ?? ''}`.toLowerCase().includes(search));
  if (q.organizationStatus && q.organizationStatus !== 'all') items = items.filter((organization) => organization.status === q.organizationStatus);
  if (q.licenseStatus && q.licenseStatus !== 'all') items = items.filter((organization) => organization.licenseStatus === q.licenseStatus);
...
class SqlOrganizationService implements IOrganizationService {
  ...
  async changeOrganizationStatus(id:string,status:OrganizationStatus){ if(status!=='active'&&status!=='suspended')throw new Error('Invalid organization status.'); try { await changeOrganizationStatus(getFirebaseClientServices().dataConnect,{id,status:status.toUpperCase() as never}); const r=await this.getOrganization(id); if(!r)throw new Error('not found'); return { ...r, status }; } catch(e){ throw safe(e,'Unable to change organization status.'); } }
}
