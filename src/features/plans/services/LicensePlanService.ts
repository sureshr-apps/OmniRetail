import { changeLicensePlanStatus, createLicensePlan, getLicensePlan, listLicensePlans, updateLicensePlan, LicensePlanStatus } from '@omniretail/sql-connect';
...
class SqlLicensePlanService implements ILicensePlanService {
  ...
  async createPlan(input: CreatePlanInput): Promise<LicensePlan> { validate(input); try { const code = planCode(input.name); await createLicensePlan(getFirebaseClientServices().dataConnect, { planCode: code, name: input.name.trim(), description: input.description?.trim() || null, level: input.level, maxStores: input.maxStores, maxUsers: input.maxUsers, auditId: newUuid(), requestId: newUuid() }); const created = (await this.rows()).find((p) => p.name === input.name.trim() && p.level === input.level); if (!created) throw new Error('created plan not found'); return created; } catch (error) { throw safeError(error, 'Unable to create the plan.'); } }
