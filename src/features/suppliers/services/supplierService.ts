...
class MockSupplierService implements ISupplierService {
  ...
  async getAllSuppliers(): Promise<Supplier[]> {
    await new Promise((r) => setTimeout(r, 40));
    return this.suppliers.map((s) => ({ ...s, recentOrders: this.getSupplierRecentOrders(s) }));
  }
}
...
export function deriveSupplierView(all: Supplier[], query: SupplierQuery = {}): SupplierQueryResult {
  ...
  suppliers = suppliers.filter((s) => (!search || `${s.supplierCode} ${s.name} ${s.contactPerson} ${s.phone} ${s.email}`.toLowerCase().includes(search)) && (!query.status || query.status === 'ALL' || s.status === query.status) && (!query.city || s.city === query.city) && (!query.category || s.category === query.category));
...
class ProductionSupplierService implements ISupplierService {
  ...
  async getSupplierById(id: string): Promise<Supplier | null> { return (await this.getAllSuppliers()).find((s) => s.id === id || s.supplierCode === id) ?? null; }
